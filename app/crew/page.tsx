    ...
    <section className="w-full min-h-screen pb-12 bg-crew-mobile md:bg-crew-tablet lg:bg-crew-desktop bg-no-repeat bg-center bg-cover text-white scroll-smooth">
      ...
      <div className=" w-10/12 mx-auto pb-12">
        <h4 className=" text-center tracking-[2.7px] font-barlow my-4 md:mb-8 md:text-start md:text-[20px] lg:tracking-[4.8px]">
          <span className=" font-bold opacity-25 mr-1">02</span>{" "}
          MEET YOUR CREW
        </h4>
        <Slider {...settings} centerMode={false} className="">
          {jsonData.crew.map((crew, index) => (
            <div className="mt-4 text-center" key={index}>
              ...
              <div>
                <p className=" mt-4 font-bellefair uppercase text opacity-50">{crew.role}</p>
                <p className="font-bellefair text-2xl uppercase">{crew.name}</p>
